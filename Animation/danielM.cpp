...
void spriteAnimation(Game *g, int *keys, const double frameRate, double timeSpan) 
{
    ...
    float attackFrame = 6.0*frameRate;
    //Draw the player
    ...
    //} else if (keys['l']) {
        ...
        if(g->player.getFwd()) {
            if(frameCountdown < (attackFrame)) {
                ...
                glTexCoord2f(0.1604f, 0.5844f); glVertex2f(-wid, -wid);
                glTexCoord2f(0.1604f, 0.538f); glVertex2f(-wid, wid-30);
                glTexCoord2f(0.2567f, 0.538f); glVertex2f(wid+140, wid-30);
                glTexCoord2f(0.2567f, 0.5844f); glVertex2f(wid+140, -wid);
