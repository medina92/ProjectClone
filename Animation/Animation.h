...
void spriteAnimation(Game *g, int *keys, const double frameRate, double timeSpan) 
{
    ...
    float attackFrame = 3.0*frameRate;
    //Draw the player
    ...
    //} else if (attack) {
        ...
        if(frameCountdown < (attackFrame)) {
            glTexCoord2f(0.0396f, 0.5827f); glVertex2f(-wid, -wid);
            glTexCoord2f(0.0396f, 0.538f); glVertex2f(-wid, wid-20);
            glTexCoord2f(0.0826f, 0.538f); glVertex2f(wid, wid-20);
            glTexCoord2f(0.0826f, 0.5827f); glVertex2f(wid, -wid);
        } else if(frameCountdown > attackFrame && frameCountdown < 2.0*attackFrame) {
            glTexCoord2f(0.0826f, 0.5842f); glVertex2f(-(wid*3), -wid);
            glTexCoord2f(0.0826f, 0.5232f); glVertex2f(-(wid*3), wid+10);
            glTexCoord2f(0.1604f, 0.5232f); glVertex2f(wid, wid+10);
            glTexCoord2f(0.1604f, 0.5842f); glVertex2f(wid, -wid);
        } else if(frameCountdown >= 2.0*attackFrame) {
            glTexCoord2f(0.1604f, 0.5844f); glVertex2f(-(wid+140), -wid);
            glTexCoord2f(0.1604f, 0.538f); glVertex2f(-(wid+140), wid-30);
            glTexCoord2f(0.2567f, 0.538f); glVertex2f(wid, wid-30);
            glTexCoord2f(0.2567f, 0.5844f); glVertex2f(wid, -wid);
            /*if(frameCountdown >= 3.0*attackFrame) {
                frameCountdown = 0.0;
                attack = 0;
            }*/
            g->player.setPosX(g->player.getPosX() - 20);
            g->player.setPosX(g->player.getPosX() + 20);
        }
        ...
        //idle stance
        if(g->player.getFwd()) {
            glTexCoord2f(0.014f, 0.055f); glVertex2f(-wid, -wid);
            glTexCoord2f(0.014f, 0.0f); glVertex2f(-wid, wid);
            glTexCoord2f(0.055f, 0.0f); glVertex2f(wid, wid);
            glTexCoord2f(0.055f, 0.055f); glVertex2f(wid, -wid);
        } else {
            glTexCoord2f(0.055f, 0.055f); glVertex2f(-wid, -wid);
            glTexCoord2f(0.055f, 0.0f); glVertex2f(-wid, wid);
            glTexCoord2f(0.016f, 0.0f); glVertex2f(wid, wid);
            glTexCoord2f(0.016f, 0.055f); glVertex2f(wid, -wid);
        }
    }
    glEnd();
    glPopMatrix();
